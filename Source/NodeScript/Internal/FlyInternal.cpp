...
namespace FLY_NAMESPACE
{
    ...
    namespace Internal
    {
        ...
        [[nodiscard]] std::vector<NodeRef>& GetNodeRefs(NodeTypeID nodeTypeID)
        {
            return GetFoundation().mNodeRefs[nodeTypeID];
        }
        ...
        void AddNode(NodeGraph& nodeGraph, Node&& node, const NodeID nodeID, CommandTracker* const commandTracker)
        {
            ...
            auto& nodeRefs = GetNodeRefs(createdNode.GetTypeID());
            nodeRefs.push_back(CreateContextualNodeRef(nodeID, nodeGraph));
            ...
            auto doCommandFunction = [](const CreateNodeData& data) -> void
                {
                    GetNode(data.nodeID, *data.nodeGraph).SetIsDestroyed(false);
                };

            auto undoCommandFunction = [](const CreateNodeData& data) -> void
                {
                    GetNode(data.nodeID, *data.nodeGraph).SetIsDestroyed(true);
                };
        ...
        void UnbindVariable(Class& flyClass, const NodeRef& nodeRef, CommandTracker* commandTracker)
        {
            ...
            {
                .nodeRef = gNodeRef,
                .varRef = GetVariableRefByNodeRef(gNodeRef)
            };

            auto doCommandFunction = [](const UnbindVarData& data) -> void
                {
                    GetFoundation().mNodeRefToVarRef.erase(data.nodeRef);
                };

            auto undoCommandFunction = [](const UnbindVarData& data) -> void
                {
                    GetFoundation().mNodeRefToVarRef[data.nodeRef] = data.varRef;
                };
        ...
        PinTypeID AddPinTypeToNodeType(const NodeTypeID nodeTypeID, const GenericDataTypeID dataTypeID, const eIODirection ioDirection, std::string pinName)
        {
            ...
            for (const NodeRef& nodeRef : GetNodeRefs(nodeTypeID))
            {
                ...
                // Create split pin as well
                std::vector<PinID>& splitPinIDs = SelectByIODirection(ioDirection, node.GetSplitInputPins(), node.GetSplitOutputPins());
                const PinID createdSplitPinID = CreatePin(nodeRef.GetNodeGraph(), nodeRef.GetNodeID(), createdPinTypeID, CreateNodeCreationContext());

                splitPinIDs.push_back(createdSplitPinID);
            }
        ...
        static void SetPinAtIndexNodeType(const NodeTypeID nodeTypeID, const std::size_t index, const GenericDataTypeID dataTypeID, const eIODirection ioDirection)
        {
            ...
            std::vector<PinTypeID>& pinTypeIDs = SelectByIODirection(ioDirection, nodeType.GetInputPinTypeIDs(), nodeType.GetOutputPinTypeIDs());
            const PinTypeID oldPinTypeID = pinTypeIDs.at(index);
            ...
            pinTypeIDs.at(index) = newPinTypeID;
