...
namespace FLY_NAMESPACE
{
	...
	void EvaluateInputValues(const std::vector<PinID>& inputPinIDs, const InternalExecutionContext& context, const size_t startIndex)
	{
		NodeGraph& currentNodeGraph = context.nodeData.nodeRef.GetNodeGraph();
		for (size_t i = startIndex; i < inputPinIDs.size(); ++i)
		{

			const PinID inputPinID = inputPinIDs[i];

			const Pin& inputPin = currentNodeGraph.GetPin(inputPinID);
			const PinType& inputPinType = context.pinTypeManager.GetPinType(inputPin.GetTypeID());
			...
			const NodeType& connectedNodeType = context.nodeTypeManager.GetNodeType(connectedNode.GetTypeID());

			if (!HasFlag(connectedNodeType.GetTraits(), eNodeTrait::HasFlow))
			{
				context.nodeExecutor.ExecuteNode(NodeExecutionData{ CreateContextualNodeRef(connectedNodeID, context.nodeData.nodeRef.GetNodeGraph()), eNodeTriggerReason::Read });
			}
