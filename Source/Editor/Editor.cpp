...
namespace Simple
{
	...
	static void SetUpDefaultLayout(std::vector<std::shared_ptr<PopUp>>& popUpWindows, std::vector<std::unique_ptr<MainMenuTabBase>>& mainMenuTabs, Engine& engine)
	{
		...
		MenuTabDefault& helpTab = AddMenuTab<MenuTabDefault>(mainMenuTabs, "Help");
		...
		std::shared_ptr<AudioSettingsPopUp> audioSettingPopUp = AddPopUpWindow<AudioSettingsPopUp>(popUpWindows, "Audio Settings");
		std::shared_ptr<CameraSettingsPopUp> cameraSettingPopUp = AddPopUpWindow<CameraSettingsPopUp>(popUpWindows, "Camera Settings");
		...
		helpCamercontrolsPopUpButton->AddCallback(EditorCallbacks::SetPopUpActive(cameraHelpPopUp, &helpCamercontrolsPopUpButton->GetIsActiveRef()));
		settingsAudioButton->AddCallback(EditorCallbacks::SetPopUpActive(audioSettingPopUp, &settingsAudioButton->GetIsActiveRef()));
		settingsCameraButton->AddCallback(EditorCallbacks::SetPopUpActive(cameraSettingPopUp, &settingsCameraButton->GetIsActiveRef()));
		settingsGraphicsButton->AddCallback(EditorCallbacks::SetPopUpActive(graphicsSettingPopUp, &settingsGraphicsButton->GetIsActiveRef()));
	...
	void Editor::Render()
	{
		...
		for (auto& popUp : mPopUpWindows)
		{
			PROFILER_BEGIN(popUp->GetImGuiName());
			if (popUp->IsActive())
			{
