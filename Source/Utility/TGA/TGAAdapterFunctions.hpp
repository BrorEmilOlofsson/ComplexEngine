...
namespace Simple
{
	...
	inline MeshData<Vertex> LoadMeshData(const TGA::FBX::Mesh& tgaMesh)
	{
		...
		for (size_t i = 0; i < tgaMesh.Elements[0].Vertices.size(); ++i)
		{
			...
			vertex.bones.x = static_cast<float>(currentVertex.BoneIDs[0]);
			vertex.bones.y = static_cast<float>(currentVertex.BoneIDs[1]);
			vertex.bones.z = static_cast<float>(currentVertex.BoneIDs[2]);
			vertex.bones.w = static_cast<float>(currentVertex.BoneIDs[3]);
			...
			vertex.weights.x = currentVertex.BoneWeights[0];
			vertex.weights.y = currentVertex.BoneWeights[1];
			vertex.weights.z = currentVertex.BoneWeights[2];
			vertex.weights.w = currentVertex.BoneWeights[3];
