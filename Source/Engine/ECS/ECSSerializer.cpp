...
namespace CLX
{
    ...
    void HandleSerializationID(EntitySerializationIDGenerator& entityIDGenerator, const EntitySerializationID& loadedSerializationID)
    {
        std::optional<Bounds<uint64_t>> usedSerializationIDBounds = entityIDGenerator.GetUsedIDBounds();
        if (usedSerializationIDBounds)
        {
            if (usedSerializationIDBounds->Contains(loadedSerializationID.id) && entityIDGenerator.IsIDUsed(loadedSerializationID))
            {
                ASSERT(false);
            }
            else
            {
                usedSerializationIDBounds->ExpandToContain(loadedSerializationID.id);
            }
        }
        else
        {
            entityIDGenerator.SetUsedIDBounds(Bounds<uint64_t>::FromMinAndMax(loadedSerializationID.id, loadedSerializationID.id));
        }
    }
