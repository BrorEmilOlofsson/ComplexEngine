...
#pragma region Views
		...
		class ConstComponentView final
		{
			...
			public:
				...
				constexpr ConstComponentIterator(const ECS* ecs, std::span<const std::span<const EntityID>> entityIDs, const unsigned int listIndex = 0)
					: mECS(ecs)
					, mEntityIDs(entityIDs)
				...
				{
					const auto& currentList = mEntityIDs[mListIndex];
					if (mEntityIndex == currentList.size() - 1)
					{
						mListIndex++;
						mEntityIndex = 0;
					}
			...
			private:
				...
				unsigned int mListIndex = 0;
				unsigned int mEntityIndex = 0;
				std::span<const std::span<const EntityID>> mEntityIDs;
				const ECS* mECS = nullptr;
			};

		public:
			constexpr ConstComponentView(const ECS* ecs, std::vector<std::span<const EntityID>>&& list)
				: mECS(ecs)
			...
			{
				return ConstComponentIterator(mECS, mEntityIDs, static_cast<unsigned int>(mEntityIDs.size()));
			}

			[[nodiscard]] std::size_t GetSize() const
			{
				std::size_t count = 0;
				for (auto& i : mEntityIDs)
				{
					count += size(i);
				}
				return count;
		...
		private:
			...
			std::vector<std::span<const EntityID>> mEntityIDs;
			const ECS* mECS = nullptr;
		};
...
#pragma endregion
	...
	inline void ECS::UpdateEntityMask(const EntityID entityID, const ComponentMask newMask)
	{
		...
		std::erase(mEntitiesByMask.at(entityData.mask), entityID);
		entityData.mask = newMask;
		mEntitiesByMask[entityData.mask].push_back(entityID);
	}
