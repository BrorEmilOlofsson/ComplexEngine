...
namespace CLX
{
    ...
    std::optional<Point3f> Navmesh::FindClosestPointInNavmesh(const Point3f& position, const AABB2f& searchField) const
    {
        const Point2f pos2D = ToPoint2XZ(position);

        // Check walls
        const std::unordered_set<NavmeshWallIndex> walls = mWallGrid.GetObjectsByAABB(searchField);
        ...
        // Check vertices
        const std::unordered_set<NavmeshVertexIndex> vertexIndices = mVertexGrid.GetObjectsByAABB(searchField);
    ...
    {
        return ToFace(mNodes[nodeIndex], mNavmeshData.m3DVertices);
    }
    ...
    {
        return IsVertexConnectedToNode(vertexIndex, mNodes[nodeIndex]);
    }

    bool Navmesh::IsVertexConnectedToNode(const NavmeshVertexIndex vertexIndex, const NavmeshNode& node) const
    {
        if (vertexIndex >= mNavmeshData.mVertices.size())
        {
            return false;
        }
        for (int i = 0; i < Vertices_In_Triangle; i++)
        {
            if (node.mVertexIndices[i] == vertexIndex)
            {
                return true;
            }
        }
    ...
    std::pair<NavmeshNodeIndex, NavmeshNodeIndex> Navmesh::GetConnectedNodes(const NavmeshVertexIndex vertexIndex1, const NavmeshVertexIndex vertexIndex2) const
    {
        std::pair<NavmeshNodeIndex, NavmeshNodeIndex> connectedNodes{ GetInvalidIndex<NavmeshNodeIndex>(), GetInvalidIndex<NavmeshNodeIndex>() };
        for (const auto& [nodeIndex, node] : mNodes | std::views::enumerate)
        {
            if (IsVertexConnectedToNode(vertexIndex1, node) && IsVertexConnectedToNode(vertexIndex2, node))
            {
                if (connectedNodes.first == GetInvalidIndex<NavmeshNodeIndex>())
                {
                    connectedNodes.first = static_cast<NavmeshNodeIndex>(nodeIndex);
                }
                else
                {
                    connectedNodes.second = static_cast<NavmeshNodeIndex>(nodeIndex);
    ...
    std::vector<NavmeshNodeIndex> Navmesh::GetConnectedNodes(const NavmeshVertexIndex vertexIndex) const
    {
        std::vector<NavmeshNodeIndex> connectedNodes;
        for (const auto& [nodeIndex, node] : mNodes | std::views::enumerate)
        {
            if (IsVertexConnectedToNode(vertexIndex, node))
            {
                connectedNodes.push_back(static_cast<NavmeshNodeIndex>(nodeIndex));
            }
        }
        return connectedNodes;
    }

    std::vector<NavmeshVertexIndex> Navmesh::GetConnectedVertices(const NavmeshVertexIndex vertexIndex) const
    {
        std::vector<NavmeshVertexIndex> connectedVertices;
        for (const auto& [nodeIndex, node] : mNodes | std::views::enumerate)
        {
            if (!IsVertexConnectedToNode(vertexIndex, node))
            {
            ...
            for (const NavmeshVertexIndex otherVertexIndex : node.mVertexIndices)
            {
                if (otherVertexIndex == vertexIndex)
                {
                    continue;
                }
                if (!std::ranges::any_of(connectedVertices, [otherVertexIndex](const NavmeshVertexIndex index) { return index == otherVertexIndex; }))
                {
                    connectedVertices.push_back(otherVertexIndex);
                }
            }
        }
    ...
    bool Navmesh::AreVerticesConnected(const NavmeshVertexIndex vertexIndex1, const NavmeshVertexIndex vertexIndex2) const
    {
        ...
        for (const NavmeshNode& node : mNodes)
        {
            if (IsVertexConnectedToNode(vertexIndex1, node) && IsVertexConnectedToNode(vertexIndex2, node))
            {
                return true;
