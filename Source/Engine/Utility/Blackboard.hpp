...
namespace CLX
{
    ...
    template<typename KeyType, typename Hasher = std::hash<KeyType>>
    class BasicBlackboardStack final
    {
    public:

        BasicBlackboardStack() = default;

        template<IsBlackboardKey<KeyType> T> requires std::is_reference_v<typename T::type>
        void Push(typename T::type value)
        {
            constexpr KeyType key = T::key;
            mMapStack[key].push(std::any(std::ref(value)));
        }

        template<IsBlackboardKey<KeyType> T> requires !std::is_reference_v<typename T::type>
        void Push(const typename T::type& value)
        {
            constexpr KeyType key = T::key;
            mMapStack[key].push(std::any(value));
        }
        ...
        {
            constexpr KeyType key = T::key;
            mMapStack[key].push(std::any(std::move(value)));
        }
        ...
        [[nodiscard]] constexpr decltype(auto) Get() const
        {
            ...
            if constexpr (std::is_reference_v<Type>)
            {
                return std::any_cast<const std::reference_wrapper<std::remove_reference_t<Type>>>(mMapStack.at(key).top()).get();
            }
            else
            {
                return std::any_cast<const Type&>(mMapStack.at(key).top());
            }
        }

    private:

        std::unordered_map<KeyType, std::stack<std::any>, Hasher> mMapStack;
    };

    using BlackboardStack = BasicBlackboardStack<std::string_view>;
}